...
# Generate data split
up_split <- initial_split(urban_parks_data, prop = 0.8)   # up ~ urban_parks
...
# Recipes
  ...
  rec_bos <- recipe(DOTHIS:: Complete Recipe)
  
  # Recipe for mean_percent_open_space_access
  rec_osa <- recipe(DOTHIS:: Complete Recipe)
  
  # Build recipe list
  recipes <- list(built_os = rec_bos,    # Built Open Space
                  os_access = rec_osa)   # Open Space Access

# Define Models
  
  EX) xx_model <- model_func() %>% 
        set_engine("model_eng") %>% 
        set_mode("regression")
  
# Compile list of models
models <- list(xx = xx_model, 
               ...)

# combine models and preproc (recipes) into a workflow set
up_wf_set <- workflow_set(
  preproc = recipes,
  models = models) %>% 
  workflow_map("fit_resamples", seed = 520, resamples = up_10cv)

# Autoplot & Rank Results
autoplot(up_wf_set)
...
```{r Evaluating Selected Model}
# Fit selected model for each output w/ workflow
  # Feature importance (vip::vip(fit)

# Evaluate selected model
predictions <- augment(DOTHIS::fit_wf_name, new_data = test) %>% 
  mutate(diff = abs(observed - .pred))   # Compute |abs| differences

metrics(predictions, truth = logC, estimate = .pred)

# Visualize predictions vs actual values for both outputs (ideally one plot but could use patchwork)
  # See in_class_demo.R for ggplot example

```

```{r Tuning Selected Model}
...
tune_model <- function(wf, resamples) {
  ...
  # Define search space with a grid
...
# Define metric set
tuning_metrics <- metric_set(rmse, rsq, mae)

# Create workflows and models for BOS and OSA
  # BOS Model Tune Setup
  tuned_bos_model <- xx_model_function() %>% 
  ...
  # OSA Model Tune Setup
  tuned_osa_model <- xx_model_function() %>% 
  set_engine("model_engine") %>% 
  set_mode("model_mode")
...
# Tune both models
bos_tuned <- tune_model(wf_bos_tune, 
                        resamples = up_10cv, 
                        metric_set = tuning_metrics)
osa_tuned <- tune_model(wf_osa_tune,
                        resamples = up_10cv,
                        metric_set = tuning_metrics)

# Collect predicitons from final fits
preditctions_bos <- collect_predictions(final_fit_bos)
predicitons_osa <- collect_predictions(final_fit_osa)
```

```{r Finalizing Tuning}
# Define custom func to finalize model fit
finalize_model <- function(tuned_results, workflow, split, metric = "mae") {
  # Select best hyperparameters for metric
  hp_best <- select_best(tuned_results, metric = metric)
  
  # Finalize the workflow using the best hyperparameters
...
# Define custom func to augment pred and calc residuals
augment_with_residuals <- function(final_fit, new_data, target_var) {
  # Augment predictions w/ new data
  augment_preds <- augment(final_fit, new_data = new_data)

  # Calulate squared residuals
  augment_preds< - augment_preds %>% 
    mutate(residual_sq = (get(target_var) - .pred)^2)
  
  return(augmented_preds)
}

# Define custon func to plot pred and residuals
...
# Run finalize_model() to find final_fit
  bos_final_fit <- finalize_model(bos_tuned, wf_bos_tune, up_split)
  osa_final_fit <- finalize_model(osa_tuned, wf_osa_tune, up_split)
  
# Augment pred + calc residuals
  bos_aug_pred <- augment_with_residuals(bos_final_fit, urban_parks_data)
  osa_aug_pred <- augment_with_residuals(osa_final_fit, urban_parks_data)
    
# Vizualizing tuning results
  # Create Plots
    bos_tuning_plots <- plot_predictions() + ggtitle("Built Open Space Tuning Results")
    osa_tuning_plots <- plot_predictions() + ggtitle("Open Space Access Tuning Results")
...
# Predicting missing data from the UN data set based on the Parkserve data set.

...
bos_dials <- extract_parameter_set_dials(tune_bos_wf) %>% 
  finalize(up_train)
...
bos_best_mae <- show_best(bos_model_params, metric = "mae", n = 1)
...
# Create parameter set
osa_dials <- extract_parameter_set_dials(tune_osa_wf) %>% 
  finalize(up_train)
...
osa_best_mae <- show_best(osa_model_params, metric = "mae", n = 1)
...
# Print bos and osa model params
print(bos_preds)
print(osa_preds)

bos_preds_plot <- ggplot(bos_preds, aes(x = .pred, y = percent_mean_built_open_space)) +
  geom_smooth(method = lm, color = "blue") +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  scale_color_gradient() +
  labs(
    title = "Actual vs. Predicted Values for % Mean Built Open SPace", 
    x = "Predicted", 
    y = "Actual")
  
osa_preds_plot <- ggplot(osa_preds, aes(x = .pred, y = percent_mean_open_space_access)) +
  geom_smooth(method = lm, color = "blue") +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  scale_color_gradient() +
  labs(
    title = "Actual vs. Predicted Values for % Mean Open Space Access", 
    x = "Predicted", 
    y = "Actual")

pre_aug_combo_plot <- bos_preds_plot | osa_preds_plot
print(pre_aug_combo_plot)
```

#### Finalize Tuning Workflows

```{r Finalize Tuning WOrkflows}
# Finalize BOS tuning wf
...
bos_augmented_preds <- bos_augmented_preds %>% 
  mutate(residual_sq = (percent_mean_built_open_space - .pred)^2)

bos_map_preds <- ggplot(augmented_preds, aes(x = .pred, y = percent_mean_built_open_space)) +
...
bos_map_resid <- ggplot(augmented_preds, aes(x = .pred, y = residual_sq)) +
  geom_point() +
...
osa_map_preds <- ggplot(augmented_preds, aes(x = .pred, y = percent_mean_open_space_access)) +
  geom_point(aes(color = .pred), size = 3, alpha = 0.8) +
  scale_color_viridis_c(name = "Predicted") +
  coord_fixed() +
  labs(title = "Map of Predicted logQmean") +
...
osa_map_resid <- ggplot(augmented_preds, aes(x = .pred, y = residual_sq)) +
  geom_point() +
  scale_color_viridis_c(name = "Residual²") +
  labs(title = "Map of Squared Residuals") +
  theme_minimal()

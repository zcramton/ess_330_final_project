...
rec_osa1 <- recipe(mean_percent_open_space_access ~ parkland_percent + percent_half_mile_walk + pop_density + parkland_area + pop_near_parks + parkland_per_capita + city_pop, data = up_train) %>%
  step_naomit(all_predictors(), all_outcomes()) %>%  # Remove missing values from predictors and outcomes
  step_YeoJohnson(all_predictors()) %>%  # Apply Yeo-Johnson transformation to numeric variables
  step_normalize(all_predictors()) %>%  # Normalize all numeric predictors
  step_corr(all_predictors(), threshold = 0.9) %>%  # Remove highly correlated predictors (threshold > 0.9)
...
rec_osa2 <- recipe(mean_percent_open_space_access ~ parkland_percent + percent_half_mile_walk + pop_density + parkland_area + pop_near_parks + parkland_per_capita + city_pop, data = up_train) %>%
  step_naomit(all_predictors(), all_outcomes()) %>%  # Remove missing values from predictors and outcomes
  step_YeoJohnson(all_numeric()) %>%  # Apply Yeo-Johnson transformation to numeric variables
  step_normalize(all_numeric()) %>%  # Normalize all numeric predictors
  step_corr(all_predictors(), threshold = 0.9)  # Remove highly correlated predictors (threshold > 0.9)
...
osa_wf_set <- workflow_set(
  preproc = list( osa1 = rec_osa1, osa2 = rec_osa2),
  models = models,
  cross = TRUE)
```
...
bos_tuned_results <- workflow_map(
  bos_wf_set,
  fn = "tune_grid",
  resamples = up_10cv,
...
osa_tuned_results <- workflow_map(
  osa_wf_set,
  fn = "tune_grid",
  resamples = up_10cv,
  grid = 10,  # Specify grid size or tuning parameters
...
rank_results(bos_tuned_results, rank_metric = "rsq", select_best = TRUE)
rank_results(osa_tuned_results, rank_metric = "rsq", select_best = TRUE)
...
DOTHIS:: Review these funcs and resolve select_best issue in chunk 10
...
bos_final_fit <- finalize_model(bos_tuned_results, wf_bos_tune, up_split)
osa_final_fit <- finalize_model(osa_tuned_results, wf_osa_tune, up_split)
